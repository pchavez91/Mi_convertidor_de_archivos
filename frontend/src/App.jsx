...
function App() {
    ...
    const fetchApiInfo = async () => {
      try {
        ...
        if (import.meta.env.DEV) {
          ...
          // Detectar bloqueo por extensiones del navegador
          if (error.code === 'ERR_BLOCKED_BY_CLIENT' || error.message.includes('ERR_BLOCKED_BY_CLIENT')) {
            console.error('🚫 BLOQUEO DETECTADO: El navegador o una extensión está bloqueando las peticiones al backend.')
            console.error('💡 Soluciones:')
            console.error('   1. Desactiva temporalmente las extensiones de bloqueo de anuncios (uBlock, AdBlock, etc.)')
            console.error('   2. Agrega localhost:8000 a la lista blanca de tu bloqueador')
            console.error('   3. Prueba en modo incógnito sin extensiones')
            console.error('   4. Verifica la configuración de privacidad del navegador')
          }
        }
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      console.error('❌ Error en conversión:', {
        message: err.message,
        code: err.code,
